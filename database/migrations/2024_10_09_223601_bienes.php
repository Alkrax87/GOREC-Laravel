...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::create('bienes', function (Blueprint $table) {
            ...
            $table->date('f_presentacion_req_inicio_bs');
            $table->date('f_presentacion_req_fin_bs');
            $table->integer('presentacion_dias_bs')->default(0);
            //Designacion de Cotizador
            $table->date('f_designacion_cotizador_inicio_bs');
            $table->date('f_designacion_cotizador_fin_bs');
            $table->integer('designacion_dias_bs')->default(0);
            //Estudio de Mercado
            $table->date('f_estudio_mercado_inicio_bs');
            $table->date('f_estudio_mercado_fin_bs');
            $table->integer('estudiomercado_dias_bs')->default(0);
            //Cuadro Comparativo
            $table->date('f_cuadro_comparativo_inicio_bs');
            $table->date('f_cuadro_comparativo_fin_bs');
            $table->integer('cuadro_comparativo_dias_bs')->default(0);
            //Elaboracion de Certificado
            $table->date('f_elaboracion_certificado_inicio_bs');
            $table->date('f_elaboracion_certificado_fin_bs');
            $table->integer('elaboracion_certificado_dias_bs')->default(0);
            //Numero Siaf
            $table->date('f_numero_Siaf_inicio_bs');
            $table->date('f_numero_Siaf_fin_bs');
            $table->integer('numero_Siaf_dias_bs')->default(0);
            //Orden de Servicio
            $table->date('f_orden_compra_inicio_bs');
            $table->date('f_orden_compra_fin_bs');
            $table->integer('orden_compra_dias_bs')->default(0);
            //Notificacion
            $table->date('f_notificacion_inicio_bs');
            $table->date('f_notificacion_fin_bs');
            $table->integer('notificacion_dias_bs')->default(0);

            // Plazo de ejecución
            $table->integer('plazo_ejecucion_dias_bs')->default(0);
            $table->date('fecha_plazo_ejecucion_bs');
            ...
            // Observaciones
            $table->text('observaciones_bs')->nullable();

            //Carta de Desestimiento
            $table->date('fecha_carta_desestimiento_bs')->nullable();

            //Entrega Bien
            $table->date('f_entrega_bien_inicio_bs');

            //Recepcion del bien
            $table->date('f_recepcion_bien_inicio_bs');

            //Patrimonizacion
            $table->date('fecha_patrimonizacion_bs');
