...
<body>
  ...
  <footer>
    <div class="footer-content">
      ...
      <div class="date-time">
        <p>Fecha: {{ date('d/m/Y') }} <span class="time" style="margin-left: 50px">Hora: {{ date('H:i:s') }}</span>
        </p>
      </div>
    </div>
  </footer>
  ...
  <div class="content">
    ...
    @foreach ($inversiones as $inversion)
      ...
      <div style="text-align: right">
        <p style="margin: 0px; padding: 0px"><span class="text-bold">Responsable:</span>
          {{ strtoupper($inversion->usuario->nombreUsuario . ' ' . $inversion->usuario->apellidoUsuario) }}</p>
      </div>
      <hr class="custom-hr">
    @endforeach
    ...
    @foreach ($especialidades as $especialidad)
      <p class="text-bold" style="margin: 0px; padding: 0px; font-size: 14px">Especialidad:
        {{ $especialidad->nombreEspecialidad }}</p>
      <p style="margin: 0px; padding: 0px"><span class="text-bold">Avance Programado:</span>
        {{ $especialidad->porcentajeAvanceEspecialidad }}% &nbsp;&nbsp; <span class="text-bold">Avance Total:</span>
        {{ $especialidad->avanceTotalEspecialidad }}%</p>
      ...
      <ul style="margin: 0px; padding: 5px 20px">
        @foreach ($especialidad->usuarios as $usuario)
          <li>{{ $usuario->nombreUsuario . ' ' . $usuario->apellidoUsuario }} <span style="font-size: 11px"> (P:
              {{ $usuario->profesiones->pluck('nombreProfesion')->implode(', ') }}) | (E:
              {{ $usuario->especialidades->pluck('nombreEspecialidad')->implode(', ') }})</span></li>
        @endforeach
      </ul>
